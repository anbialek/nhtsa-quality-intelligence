...
def fetch_complaints(make, model, year):
    '''
    Fetch complaints from NHTSA API for specific vehicle combination
    Return dictionary with keys: records, status_code, source_url, query_params
    '''
    params = {"make": make, "model": model, "modelYear": year}
    response = requests.get(COMPLAINTS_URL, params=params, timeout=HTTP_TIMEOUT_SECONDS)
...
def write_to_bronze(records, ingest_date):
    ...
    '''
    folder = BRONZE_DIR / "complaints" / f"ingest_date={ingest_date}"
...
def main():
    logger.info("Start Complaints ingestion")
    ...
    for make, model, year in INGESTION_TARGETS:
        try:
            response = fetch_complaints(make, model, year)
